...
/* Output : The server prompts, ie, the error/warning/help messages.      */
/* Method : A well planned execution of Linux function/system calls     */
/* Possible Further Improvements :  
          1. User account creation/authentication -not done, since it is tftp type only
          2. A prompt -done
          3. A banner -done
          ...
          8. Add support for non-unix systems -not done
*/
...
void createPasswordFile()
{
  password_fp = (FILE *)fopen("passwd.lst", "a");
}
...
void addUser()
{
  ...
  printf("Note : The default password is the same as username. To change press 1, else press 2:\n");
  scanf("%d", &chosen_option);
  while (getchar() != '\n');  
